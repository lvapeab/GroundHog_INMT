...
if state['videos'] == True :
    from groundhog.datasets.TM_dataset_vid import PytablesBitextIterator
else :
    from groundhog.datasets.TM_dataset_img import PytablesBitextIterator
...
def create_padded_batch(state, x, y, return_dict=False):
    ...
    * len(x[0][idx]) is the size of sequence idx
    ...
    # if state['trim_batches']:
        ...
        # mx = numpy.minimum(state['seqlen'], max([len(xx) for xx in x[0]]))
    ...
    n = x[0].shape[0]
    if state['videos'] == True :
        X = numpy.zeros((mx, n, state['n_features_source']), dtype='float32')
    else :
        X = numpy.zeros((mx, n), dtype='float32')
    ...
    # print "len (x[0])", len (x[0]), "len (y[0])", len (y[0])
    ...
    # Unknown words
    # X[X >= state['n_features_source']] = state['unk_sym_source']
...
class EncoderDecoderBase(object):
    ...
    def _create_embedding_layers(self):
        ...
        self.approx_embedder = MultiLayer(
            self.rng,
            n_in=self.state['n_features_source']
    ...
    def _create_transition_layers(self):
        ...
        if rec_layer == RecurrentLayerWithSearch:
            logger.debug("__using_RecurrentLayerWithSearch")
        ...
        for level in range(self.num_levels):
            logger.debug("__appending_level " + str(level))
...
class RNNEncoderDecoder(object):
    ...
    def build(self):
        ...
        self.sampling_x = TT.fvector("sampling_x")
    ...
    def create_lm_model(self):
        ...
        self.lm_model = LM_Model(
            ...
            indx_word_src= None, # self.state['indx_word'],
            rng=self.rng)
...
def parse_input(state, word2idx, line, raise_unk=False, idx2word=None, unk_sym=-1, null_sym=-1):
    ...
    for idx,sx in enumerate(seqin):
        ...
        if seq[idx] >= state['n_features_source']:
            seq[idx] = unk_sym
