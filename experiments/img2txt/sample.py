...
from experiments.img2txt.numpy_compat import argpartition
...
class BeamSearch(object):
    ...
    def compile(self):
        logger.debug("Compiling representation_computer")
        self.comp_repr = self.enc_dec.create_representation_computer()
        logger.debug("Compiling initializers")
        self.comp_init_states = self.enc_dec.create_initializers()
        logger.debug("Compiling next_probs_computer")
        self.comp_next_probs = self.enc_dec.create_next_probs_computer()
        logger.debug("Compiling next_states_computer")
...
def main():
    ...
    if args.source and args.trans:
        ...
        fsrc = h5py.File(args.source, 'r')
        ftrans = open(args.trans, 'w')

        if args.verbose :
            print "Translating", fsrc['indices'].shape[0],"instances"
        start_time = time.time()

        n_samples = args.beam_size
        total_cost = 0.0
        logging.debug("Beam size: {}".format(n_samples))
        seq = numpy.zeros(state['n_features_source'], dtype='float32')

        for i, line in enumerate(fsrc[u'phrases']):
            seq[i%state['n_features_source']] = numpy.float32(line)
            if i % state['n_features_source'] ==  state['n_features_source'] - 1 :
