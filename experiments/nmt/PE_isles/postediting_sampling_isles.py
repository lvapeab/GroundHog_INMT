    ...
    def search(self, seq, n_samples, fixed_words= {}, isles = [], max_N = None, minlen=1, verbose=False, idx2word=None):
        ...
        while k < (3 * len(seq)):
            ...
            if len(unfixed_isles) == 0 or k in fixed_words: # There are no remaining isles. Regular decoding.
                ...
                new_states = [numpy.zeros((n_samples, dim), dtype="float32") for level in range(num_levels)]
                ...
                while not stop and len(unfixed_isles) > 0:
                    ...
                    logger.log(2,"Isle: %s\n"%str(map (lambda x: idx2word[x] if idx2word.get(x) is not None else self.unk_id, next_isle)))
                    ...
                    for word in next_isle:
                        if fixed_words.get(k_counter) is None:
                            ...
                            logger.log(2,"\t > Word %s (%d) will go to position %d"%(idx2word[word] if idx2word.get(word) is not None else self.unk_id, word, k_counter))
                        k_counter += 1
                    del unfixed_isles[0]
                k += best_n_words - 1
            k+=1
