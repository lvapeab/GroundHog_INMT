...
class BeamSearch(object):
    ...
    def search(self, seq, n_samples, fixed_words= {}, isles = [], max_N = None, threshold = -4500.,
        ...
        while k < (3 * len(seq)):
            ...
            if k in fixed_words: # This position is fixed by the user
                ...
                if len(unfixed_isles) == 0: # There are no remaining isles. Regular decoding.
                    ...
                    next_isle = unfixed_isles.pop(0)[1]
                    target_word = next_isle[0]
                    #logger.debug("Target_word: %s (%d)"%(idx2word[target_word],target_word))
                    target_distribution = numpy.zeros(log_probs.shape[1],dtype="float32")
                    target_distribution -= inf
                    target_distribution[target_word] = -0.1
                    maximum_words = k + max_N
                    continue_loop = True
                    while continue_loop:
                        if k >= maximum_words:
                            continue_loop = False
                        ...
                        for beam_hyp in range(log_probs.shape[0]):
                            model_distribution = log_probs[beam_hyp]
                            kl_divergence = smoothed_kl(target_distribution, model_distribution)
                            #logger.debug("kl_divergence %s"%str(kl_divergence))
                    ...
                    for n_words in range(len(hyp_costs)):
                        for beam_index in range(len(hyp_costs[n_words])):
                            normalized_cost = hyp_costs[n_words][beam_index]/n_words
                            if normalized_cost < min_cost:
                                min_cost = normalized_cost
                                n_words = n_words
                                best_hyp = hyp_trans[n_words][beam_index]

                    #trans = hyp_trans[n_words]
                    #costs = hyp_costs[n_words]
                    #states = states_[n_words+1]
                    #logger.debug("Adding %d words to position %d"%(n_words+1, k))
                    ...
                    #k += n_words
                    k_counter = k + n_words + 1
                    #logger.debug("Generated up to position %d"%k)

                    # We fix the words of the next isle
                    #logger.debug("Fixing words from the isle: %s"%str(next_isle))
                    ...
                    for i in range(len(best_hyp)):
                        if any(map(lambda x: x == best_hyp[i:], isle_prefixes)):
                            k_counter = k + i
                            #logger.debug("Overlapping hypothesis and isle")
                            #logger.debug("Hypothesis:%s"%str(best_hyp))
                            #logger.debug("Isle:%s"%str(next_isle))
                            break


                    #logger.debug("Fixed_words: %s"%str(fixed_words))
                    #logger.debug( "Upating...")
                    for word in next_isle:
                        #if k_counter not in fixed_words.keys():
        ...
        if not len(fin_trans):
            if ignore_unk:
                ...
                return self.search(seq, n_samples, fixed_words=fixed_words,isles = isles, threshold=threshold,
                ...
                return self.search(seq, n_samples * 2, fixed_words=fixed_words, isles = isles, threshold=threshold,
...
def sample(lm_model, seq, n_samples, fixed_words={}, max_N = -1, isles = [],threshold=-10.5,
    ...
    if beam_search:
        ...
        trans, costs = beam_search.search(seq, n_samples, fixed_words=fixed_words, isles = isles, threshold=threshold,
                                          max_N= max_N, ignore_unk=ignore_unk, minlen=minlen, verbose=verbose,
                                          idx2word=idx2word)
        if normalize:
...
def parse_args():
    ...
    parser.add_argument("--threshold",
                        type=float, default=-3000, help="KL threshold for stop the generation of words")
...
def main():
    ...
    logger.info('max_n: %d'%args.max_n)
            ...
            if args.interactive:
                ...
                for n_line, line in enumerate(fsrc):
                    ...
                    while not validated_hypothesis:
                        ...
                        sentences, costs, _ = sample(lm_model, seq, n_samples, max_N=max_N, isles = isles,
                                                 fixed_words=fixed_words_user, threshold=threshold,
                ...
                for n_line, line in enumerate(fsrc):
                    ...
                    if args.verbose:
                        ...
                        logger.debug("Desired translation: %s" % target_lines[n_line])
                    ...
                    sentences, costs, trans = sample(lm_model, seq, n_samples, sampler=sampler, threshold=threshold,
                    ...
                    if args.verbose:
                        logger.debug("First hypothesis: %s\n"%(" ".join(hypothesis)))
                        ...
                        #   3. Test this new hypothesis
                            ...
                            while checked_index_r < len(reference):  # We check all words in the reference
                                if checked_index_h >= len(hypothesis):
                                    ...
                                    if args.verbose:
                                        logger.debug('Adding word "%s" to position %d in fixed_words'%(reference[checked_index_r], checked_index_h))
                                        logger.debug("%s"%str(fixed_words_user))
                                    checked_index_h += 1
                                    checked_index_r += 1
                            ...
                            logger.debug("Generating a new hypothesis")
                            sentences, costs, _ = sample(lm_model, seq, n_samples, threshold=threshold,
                                                 fixed_words=copy.copy(fixed_words_user), max_N = max_N, isles = isle_indices,
                                                 sampler=sampler, beam_search=beam_search, ignore_unk=args.ignore_unk,
                                                 normalize=args.normalize, verbose=args.verbose, idx2word=indx2word_trg)
                            n+=1

                            logger.debug("Sample number %d" % n)
                            best = numpy.argmin(costs)
                            hypothesis = sentences[best].split()
                            if args.verbose:
                                logger.debug("\tSentence %d. Hypothesis %d: %s" % (n_line+1, hypothesis_number, " ".join(hypothesis)))
                    ...
                    if args.verbose:
                        ...
                        logger.debug("%d errors" % errors_sentence)
