...
class BeamSearch(object):
    ...
    def search(self, seq, n_samples, ngram_model, lm_model, lambda_weight = 0.5,  ignore_unk=False, minlen=1):
...
def sample(lm_model, seq, n_samples, ngram_model,
        sampler=None, beam_search=None, lambda_weight=0.5,
        ignore_unk=False, normalize=False,
        alpha=1, verbose=False):
    if beam_search:
        sentences = []
        trans, costs = beam_search.search(seq, n_samples, ngram_model, lm_model, lambda_weight=lambda_weight,
...
def parse_args():
    ...
    parser.add_argument("--ngram",
            type=str, help="Ngram model to use")
    parser.add_argument("--lamb",
            type=float, default=0.5, help="weight given to the continuous model")
...
def main():
    ...
    logger.debug('Building Encoder-Decoder model')
    ...
    logger.debug('Loading ngram language model')
    ngram_model = kenlm.LanguageModel(args.ngram)
    lambda_weight = args.lamb
    logger.debug('Using lambda = %s'%lambda_weight)
    ...
    if args.source and args.trans:
        ...
        for i, line in enumerate(fsrc):
            ...
            trans, costs, _ = sample(lm_model, seq, n_samples, ngram_model, sampler=sampler, lambda_weight=lambda_weight,
