...
def parse_args():
    ...
    parser.add_argument("--input", help="Source phrases")
    parser.add_argument("--output", help="Save representations to")
    ...
    parser.add_argument("--allow-unk", default=True, action="store_true",
    ...
    if args.mode == "batch":
        ...
        while True:
            try:
                ...
                compute_reprs = enc_dec.create_representation_computer()
                ...
                print "Reprs:", reprs[0]
                print "Reprs shape:", reprs[0].shape
            except Exception:
                traceback.print_exc()
    elif args.mode == "save_representations":
        assert args.input and args.output
        src_file = open(args.input, "r")
        if args.output[-3:] == 'txt':
            hdf5 = False
        else:
            hdf5 = True
        if hdf5:
            logger.debug("Saving representations to %s in hdf5 format", args.output)
            f = tables.open_file(args.output, "w")
            atom = tables.Float32Atom()
            representations = f.create_earray(f.root, 'reprs', atom, (0, state['dim']))

        else:
            logger.debug("Saving representations to %s (text mode)", args.output)
            f = open(args.output, "w")
        compute_reprs = enc_dec.create_representation_computer()
        logger.debug("Collecting representations...")
        try:
            i = 0
            while True:
                ...
                if hdf5:
                    ...
                    sentence_representation.tofile(f, sep=",")
                    f.write('\n')
                i += 1
                if (i + 1)  % 100 == 0:
                    logger.debug("%d sentences processed"%(i+1))
        except StopIteration:
