    ...
    def __call__(self):
        ...
        while sample_idx < self.state['n_examples']:
            ...
            for seq_idx in range(xs.shape[1]):
                ...
                x_words = cut_eol(map(lambda w_idx : self.model.source_language.indx_word[w_idx], x))
                y_words = cut_eol(map(lambda w_idx : self.model.target_language.indx_word[w_idx], y))
                if len(x_words) == 0:
                    continue
