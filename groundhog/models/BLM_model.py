...
class BLM_Model(Model):
    def  __init__(self,
        ...
        """
        super(BLM_Model, self).__init__(output_layer=cost_layer,
                                       sample_fn=sample_fn,
                                       indx_word=indx_word,
    ...
    def validate_phrases(self, test_iter, train=False) :
        cost = 0
        n_batches = 0
        n_steps = 0
        sentence_i =0
        ...
        sentence = ""
        len_sentence = 0
        data_dict = numpy.load(self.indx_word)
        self.word_indxs = data_dict['unique_words']
        ...
        while  True :
            ...
            vals = { 'x':inputs.flatten(), 'y':numpy.append(inputs[1:],0),'reset':0.0,}
            #print vals
            n_batches += 1
            if isinstance(vals, dict):
                ...
                if self.test_verbosity == 1 : #and data_iterator.seq_len == 1 :
                    sentence = ""
                    for word in vals['x'] :
                        sentence+= self.word_indxs[word]+ " "
                    print sentence, "- -n_steps",n_steps, "-log-prob", _rvals

                cost +=_rvals/numpy.log(n_steps)
                print -_rvals/n_steps
        ...
        log_prob = -cost # (numpy.log(2.))
        ppl = 10**(numpy.log(2)*cost/numpy.log(10))

        return [('cost',log_prob), ('ppl',ppl)]
