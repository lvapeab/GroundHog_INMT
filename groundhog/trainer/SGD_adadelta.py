...
class AdaGrad(object):
    ...
    def __init__(self,
        ...
        #self.gs = [theano.shared(numpy.zeros(p.get_value(borrow=True).shape, dtype=theano.config.floatX), name=p.name)
        ...
        # θ{t+1,i}=θ{t,i}−\div{η}{\sqrt{G_{t,ii}+ϵ}}*g_{t,i}
        weight_update_list = [(weight, weight - lr * gradient/
                               shift_zeroes(TT.sqrt(TT.inc_subtensor(G_t[1][:], 1e-8) + TT.pow(gradient, 2))))
                              for weight, gradient, G_t
        ...
        self.update_fn = theano.function(model.inputs + [lr], [], name='update_function',
