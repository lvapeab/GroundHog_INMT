...
def grab_text(path, filename, vocab, oov_default, dtype, level):
    ...
    if level == 'words':
        ...
        return numpy.array(
            ...
            dtype=dtype)
"""
def grab_text_reversed(path, filename, vocab, oov_default, dtype, level):
    filename = os.path.join(path, filename)
    fd = open(filename, 'rt')
    txt = fd.read()
    if level == 'words':
        ...
        txt = txt.replace('  ', ' ')
        txt = txt.split(' ')
        txt = [x for x in reversed(txt) if x != '']
        return numpy.asarray(
            [vocab.get(w, oov_default) for w in reversed(txt)],
            dtype=dtype)
    else:
        return numpy.array(
            [vocab.get(w, oov_default) for w in reversed(txt)],
...
#        data = {'train_words': train, 'valid_words': valid, 'test_words': test, 'train_words_reversed': train_rev, 'valid_words_reversed': valid_rev, 'test_words_reversed': test_rev,'n_words': len(vocab)}
    else:
