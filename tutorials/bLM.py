...
def get_state():
    ...
    state['seqlen'] = 81
    state['path'] = "/home/lvapeab/smt/software/GroundHog/tutorials/DATA/miniue/fda/miniue.npz"
    state['dictionary'] = "/home/lvapeab/smt/software/GroundHog/tutorials/DATA/miniue/fda/miniue_dict.npz"
    state['chunks'] = 'words'
    state['seed'] = 123
    state['debug'] = True
    state['on_nan'] = 'warn'
...
def jobman(state, channel):
    ...
    outhid = outhid_activ(forward_training.emb_state(forward_training.rec_layer) +
                          backward_training.emb_state(backward_training.rec_layer)
                          +
                          forward_training.emb_words_out(x) +
    ...
    def sample_fn(word_tm1, h_tm1):
        ...
        outhid = outhid_dropout(outhid_activ(forward_valid.emb_state(h0, use_noise=False, one_step=True) +
                                             forward_valid.emb_words_out(word_tm1, use_noise=False, one_step=True),
                                             one_step=True) +
                                outhid_activ(backward_valid.emb_state(h0_b, use_noise=False, one_step=True) +
